...
public class PolandNotation {
    public static void main(String[] args) {
        ...
        String suffixExpression = "3 4 + 5 * 6 -"; // 76
        //思路
    ...
    public static List<String> parseSuffixExpreesionList(List<String> list) {
        ...
        for (String item : list) {
            ...
            if (item.matches("\\d+")) {
                ...
                //问题： 我们缺少一个比较优先级高低的方法
                while (s1.size() != 0 && com.eknaij.stack.Operation.getValue(s1.peek()) >= com.eknaij.stack.Operation.getValue(item)) {
                    s2.add(s1.pop());
                }
                //还需要将 item 压入栈
                s1.push(item);
